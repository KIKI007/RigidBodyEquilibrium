...
public:
    ...
    bool compute2DPolygonsIntersection(const PolyVector2 &polyA, const PolysVector2 &polyBs, PolysVector2 &polyIntsec);

    bool compute2DPolygonsIntersection(const PolyVector2 &polyA, const vector<PolysVector2> &polyBs, PolysVector2 &polyIntsec);
...
template<typename Scalar>
bool PolyPolyBoolean<Scalar>::compute2DPolygonsIntersection(const PolyPolyBoolean::PolyVector2 &polyA,
                                                            const PolyPolyBoolean::PolysVector2 &polyBs,
                                                            PolyPolyBoolean::PolysVector2 &polyIntsec) {
    PolysVector2 polys;
    polys.push_back(polyA);
    Scalar Scale = computeScale(polys);
    ...
    ClipperLib::Paths pathBs;
    {
        for(auto &polyB : polyBs)
        {
            ...
            pathBs.push_back(pathB);
        }
    }

    ClipperLib::Clipper solver;
    ClipperLib::Paths path_subject;
    path_subject.push_back(pathA);
    for(int id = 0; id < pathBs.size(); id++){
        solver.Clear();
        solver.AddPaths(path_subject, ClipperLib::ptSubject, true);
        solver.AddPath(pathBs[id], ClipperLib::ptClip, true);
        solver.Execute(ClipperLib::ctIntersection, path_subject, ClipperLib::pftPositive, ClipperLib::pftPositive);
        ClipperLib::SimplifyPolygons(path_subject);
        if (path_subject.empty())
            return false;
    }

    for(size_t id = 0; id < path_subject.size(); id++)
    {
        PolyVector2 polylist;
        for(int jd = 0; jd < path_subject[id].size(); jd++){
            int X = path_subject[id][jd].X;
            int Y = path_subject[id][jd].Y;
            polylist.push_back(Vector2((Scalar)X / Scale, (Scalar)Y / Scale));
        }
        polyIntsec.push_back(polylist);
    }
...
template<typename Scalar>
bool PolyPolyBoolean<Scalar>::compute2DPolygonsIntersection(const PolyVector2 &polyA, const vector<PolysVector2> &polyBs, PolysVector2 &polyIntsec){
    ...
    vector<ClipperLib::Paths> pathsBs;
    {
        for(int id = 0; id < polyBs.size(); id++)
        {
            ClipperLib::Paths pathsB;

            for(auto &polyB : polyBs[id])
            {
                ClipperLib::Path pathB;
                for(size_t jd = 0; jd < polyB.size(); jd++)
                {
                    int x = polyB[jd].x() * Scale;
                    int y = polyB[jd].y() * Scale;
                    pathB.push_back(ClipperLib::IntPoint(x, y));
                }
                if(!ClipperLib::Orientation(pathB))
                {
    ...
    ClipperLib::Paths path_subject;
    path_subject.push_back(pathA);
    for(int id = 0; id < pathsBs.size(); id++)
    {
        solver.Clear();
        solver.AddPaths(path_subject, ClipperLib::ptSubject, true);
        solver.AddPaths(pathsBs[id], ClipperLib::ptClip, true);
        solver.Execute(ClipperLib::ctIntersection, path_subject, ClipperLib::pftPositive, ClipperLib::pftPositive);
        ClipperLib::SimplifyPolygons(path_subject);
        if (path_subject.empty())
            return false;
