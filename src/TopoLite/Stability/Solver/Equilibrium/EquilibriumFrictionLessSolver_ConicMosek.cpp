...
#include "EquilibriumFrictionLessSolver_ConicMosek.h"


template<int dim>
void EquilibriumFrictionLessSolver_ConicMosek<dim>::unpackStaticData(const vector<double> &xx,
...
template<int dim>
void EquilibriumFrictionLessSolver_ConicMosek<dim>::unpackKinematicData(const vector<double> &velocities,
                                                              shared_ptr<KinematicData> &data) {
    getKinetamticData(data, velocities);
}

template<int dim>
bool EquilibriumFrictionLessSolver_ConicMosek<dim>::isEquilibrium_Constraints(Vector3 gravity, shared_ptr<StaticData> &data){
...
template<int dim>
bool EquilibriumFrictionLessSolver_ConicMosek<dim>::isInterlocking(Eigen::Matrix<double, Eigen::Dynamic, 1> &force, shared_ptr<KinematicData>  &data){
...
//            r = MSK_linkfunctotaskstream(task, MSK_STREAM_LOG, NULL, printstr);
            ...
            if (r == MSK_RES_OK) {
                ...
                for (int i = 0; i < NUMVAR; i++) {
                    MSK_putcj(task, i, force(i));
                }

                for(int i = 0; i < NUMVAR; i++){
                    MSK_putqobjij(task, i, i, 1);
                }
            }
                ...
                if (r == MSK_RES_OK) {
                    ...
                    switch (solsta) {
                        ...
                        case MSK_SOL_STA_OPTIMAL:
                            ...
                            if(!silence) std::cout << "Infeasible Energy:\t" << -primalobj << std::endl;
...
template<int dim>
bool EquilibriumFrictionLessSolver_ConicMosek<dim>::isEquilibrium_Dual(Vector3 gravity, shared_ptr<KinematicData>  &data){
    Eigen::MatrixXd A, Bint;
...
template<int dim>
bool EquilibriumFrictionLessSolver_ConicMosek<dim>::isEquilibrium(Vector3 gravity, shared_ptr<StaticData> &data)
{
...
template class EquilibriumFrictionLessSolver_ConicMosek<3>;
template class EquilibriumFrictionLessSolver_ConicMosek<2>;
