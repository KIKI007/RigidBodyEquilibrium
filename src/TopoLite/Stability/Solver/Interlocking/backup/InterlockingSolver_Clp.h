...
public:
    InterlockingSolver_Clp(pContactGraph _graph,
            CLP_SOLVER_TYPE _type = SIMPLEX): InterlockingSolver<Scalar>::InterlockingSolver(_graph), type(_type)
    {
...
public:
    ...
    bool isTranslationalInterlocking(pInterlockingData &data) override;

    bool isRotationalInterlocking(pInterlockingData &data) override;
