...
void PolyMesh<Scalar>::fromEigenMesh(const MatrixX &V, const MatrixX &T, const MatrixXi &F){
    ...
    for(int id = 0; id < F.rows(); id++){
        ...
        for(int jd = 0; jd < 3; jd++){
            ...
            if(T.rows() > 0) polygon->texs.push_back(textureList[F(id, jd)]);
        }
...
template<typename Scalar>
void PolyMesh<Scalar>::fromTriLists(vector<shared_ptr<Triangle<Scalar>>> triLists)
{
    ...
    for(auto tri: triLists)
    {
        ...
        {
            polygon->push_back(tri->v[kd]);
        }
        polygons.push_back(polygon);
    }

    setPolyLists(polygons);
    return;
}
...
template<typename Scalar>
PolyMesh<Scalar>::PolyMesh(const PolyMesh &_mesh): MeshBase<Scalar>::MeshBase(_mesh.getVarList())
{
    clear();
...
Matrix<Scalar, 3, 1> PolyMesh<Scalar>::centroid() const
{
    // Convert into triangular mesh
    vector<pTriangle> triList;
    convertToTriLists(triList, false, true);
...
template<typename Scalar>
Matrix<Scalar, 3, 1> PolyMesh<Scalar>::center() const{
...
template<typename Scalar>
void PolyMesh<Scalar>::computeBoundaryLoops(vector<shared_ptr<_Polygon<Scalar>>> &polys){
    MatrixXi F, SF;
    MatrixX V, SV;
    Eigen::VectorXi C, SVI, SVJ;
    convertPosToEigenMesh(V, F, C);

    igl::remove_duplicate_vertices(V, F, 1e-5, SV, SVI, SVJ, SF);

    std::vector<std::vector<int> > L;
    igl::boundary_loop(SF, L);
...
template<typename Scalar>
void PolyMesh<Scalar>::convertToTriLists(vector<pTriangle> &triList, bool compute_normal, bool convex) const
{
    ...
    for (size_t i = 0; i < polyList.size(); i++)
    {
        ...
        if(convex) poly->triangulateNaive(triangles);
        else poly->triangulate(triangles);
