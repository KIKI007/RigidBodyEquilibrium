...
public:
    ...
    void setVertices2(vector<Vector2> _vers);

    size_t push_back(VPoint<Scalar> pt);
    ...
    void removeDuplicatedVertices(double eps = FLOAT_ERROR_SMALL);
...
public:
    ...
    shared_ptr<_Polygon<Scalar>> projectPolygonInto2D(Vector3 &x_axis, Vector3 &y_axis, Vector3 &origin) const;
...
public:
    ...
    void triangulate(vector<pTriangle> &tris,
                     std::string settings = "Q",
                     bool input_triangle = false) const;

	int getPtVerID(Vector3 point) const;

    vector<Line<Scalar>> convertToLines() const;

    bool checkInsidePolygon(Vector3 pt);

    void triangulateQuad(int num_x, int num_y, vector<shared_ptr<Triangle<Scalar>>> &tris);
