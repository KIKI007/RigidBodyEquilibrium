...
TEST_CASE("PolyMesh - Four Quad with index as input") {
    ...
    SECTION("Per Vertex Normal"){
        vector<shared_ptr<Triangle<double>>> triLists;
        polyMesh.convertToTriLists(triLists, true);
        REQUIRE((triLists[0]->vf[0] - Vector3d(0, 0, 1)).norm() == Approx(0));
    }
}
