...
TEST_CASE("GeometricPrimitives Line"){
    Line<double> lineA(Vector3d(0, 0, 0), Vector3d(1, 0, 0));
    REQUIRE(lineA.distance3(Vector3d(0, 1, 0)) == 1.0);

    SECTION("cover"){
        Line<double> lineB(Vector3d(-1, 0, 0), Vector3d(2, 0 ,0));
        REQUIRE(lineA.checkIntersection(lineB) == 1);
    }

    SECTION("half intersect"){
        Line<double> lineB(Vector3d(0.5, 0, 0), Vector3d(2, 0 ,0));
        REQUIRE(lineA.checkIntersection(lineB) == 1);
    }

    SECTION("include"){
        Line<double> lineB(Vector3d(0.5, 0, 0), Vector3d(0.8, 0 ,0));
        REQUIRE(lineA.checkIntersection(lineB) == 1);
    }

    SECTION("seperate but parallel"){
        Line<double> lineB(Vector3d(-0.8, 0, 0), Vector3d(-0.5, 0 ,0));
        REQUIRE(lineA.checkIntersection(lineB) == 2);
    }
}
...
TEST_CASE("GeometricPrimitives Plane")
{
    ...
    SECTION("checkAtSameHalfSpace"){
        vec = Vector3d(1.1e-7, 0 ,0);
        REQUIRE(plane.checkAtSameHalfSpace(vec) == POINT_PLANE_POSITIVE_SIDE);
        vec = Vector3d(-1.1e-7, 0 ,0);
        REQUIRE(plane.checkAtSameHalfSpace(vec) == POINT_PLANE_NEGATIVE_SIDE);
        vec = Vector3d(0.9e-7, 0 ,0);
        REQUIRE(plane.checkAtSameHalfSpace(vec) == POINT_PLANE_INTERSECT);
        vec = Vector3d(-0.9e-7, 0 ,0);
        REQUIRE(plane.checkAtSameHalfSpace(vec) == POINT_PLANE_INTERSECT);
    }
