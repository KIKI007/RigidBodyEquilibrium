...
#include "Stability/ContactGraph/ContactGraphPlane.h"
...
_Polygon<double> buildContactPolygon(_Polygon<double>contactPolygon, const shared_ptr<ContactGraphPlane<double>>& graph) {
...
TEST_CASE("Class ContactGraph") {
    ...
    SECTION("load two square A,B. A's normal is [0,0,1], B's normal is [0,0,-1]") {
        ...
        shared_ptr<ContactGraphPlane<double>> graph = make_shared<ContactGraphPlane<double>>(varList);
